...
const BookRecGenreTitle = ({userId}) => {
  ...
  const handleRecommend = async () => {
    if (selectedBook) {
      try {
        ...
        const genre  = selectedBook.genres.slice(1, -1).replace(/'/g, "").split(', '); 
        console.log(genre[0])
        setLoading(true);
        // Call your backend API for recommendations based on selectedBook.title and selectedBook.author
        const response = await axios.get(
          `http://localhost:8000/api/get_recommendations_genre/?title=${selectedBook.title}&genre=${genre[0]}`
        );
...
export default BookRecGenreTitle;